...
Icon values are Font Awesome 4 unicode codepoints. **Must use FA4 codepoints only** - FA5/FA6 codepoints will not display.

Reference: https://fontawesome.com/v4/icons/

**Common home automation icons (FA4):**
...
WebHASP looks for credentials in this order:

1. **localStorage** (per device) - set via the setup screen, takes priority
2. **Device config file** - `ha.url` and `ha.token` in the device JSON
3. **Setup screen** - shown if neither of the above are found

The config file option is useful for getting onto devices where typing a long token is impractical (old iPads, TVs). Once credentials are entered via the setup screen they are stored in localStorage and the config file values are no longer used.
...
**Security note:** Storing a token in a config file in the `www/` folder means it is accessible to anyone on your local network. For most home setups this is acceptable. If you share your network with untrusted devices, use the setup screen instead.
